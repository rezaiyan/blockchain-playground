...
class BlockchainController {
            ...
            if (req.params.address) {
                ...
                try {
                    ...
                    if (stars) {
                        ...
                        return res.status(404).send("No star for this address!");
                    }
                } catch (error) {
    ...
    // This endpoint allows you validate the chain if there is any errors
    validateChain() {
        this.app.get("/validateChain", async (_req, res) => {
            try {
                let errors = await this.blockchain.validateChain();
                if (errors.length == 0) {
                    return res.status(200).send('No Error, All Good!');
                } else {
